...
def save_kpts(annotation_idx, annotation_list):
    ...
    np.save('./render_kpts_output/keypoints/%05d.npy'%annotation_idx, np_annotations)
...
def render_frame(frame, render_offset=0, step=10, filename="%05d.jpg", folder="render_kpts_output/images", annot=True, num_knots=1, mapping=None):
    # DOMAIN RANDOMIZE
    global rig
    randomize_camera()
    ...
    #    texture_randomize(table, 'dr_data/background')
        ...
        #color_randomize(table, color_range=(0,0.1)) #0 is black 
        #define dark image for table
        #texture_randomize(table, 'dr_data/black_cloth_backgrounds')
        texture_randomize(table, 'dr_data/dark_val2017')
    if random.random() < 0.5:
        #color_randomize(rig, color_range=(0.85,1)) #rig is rope, table is background
        ...
        #texture_randomize(rig, 'dr_data/yellow_textures')
...
def render_mask(frame, render_offset=0, step=10, mask_filename="%05d", depth_filename="%05d", folder_mask="render_kpts_output/image_masks", folder_depth="render_kpts_output/image_depths"):

...
#    os.mkdir('./reordered/images_depth')
    ...
    #new_knots_info = {}
    i = 0
    for filename in sorted(os.listdir('./{}/blacked_out'.format(args.dir))):
...
#            depth = cv2.imread('./%s/images_depth/%s'%(args.dir, depth_filename)).copy()
...
#            cv2.imwrite('./reordered/images_depth/{}'.format(save_depth_filename), depth)
        i += 1
        # except:
        #     pass
